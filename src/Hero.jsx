...
const Hero = () => {
  return (
    <div className='w-full h-fit sm:h-130  flex flex-col sm:flex-row items-center gap-3 sm:gap-6  p-3 '>
        <div className='bg-secondary/ flex-1 h-full flex flex-col '>
          <h1  style={{ fontFamily: 'Poppins, sans-serif' }} className='text-ui-color text-5xl font-semibold flex items-center gap-2 border-2 border-ui-color/10  p-2 rounded-3xl w-full sm:w-fit justify-center sm:justify-start'>Full-Stack <QuestionMarkCircleIcon className='size-12 text-text-third'/></h1>
          <h1  style={{ fontFamily: 'Poppins, sans-serif' }} className='text-text text-5xl ml-1 mt-1 font-semibold  text-center sm:text-left '>Web Developer & <span className=' text-text-third'>Creative Designer Visionary •  </span> </h1>
          <p className='mt-4 mb-1 text-text-third   text-lg text-center sm:text-left'>I design and build fast, modern, and scalable <b>web applications</b>.My main expertise is <b>frontend and backend development</b>, supported by strong skills in <b>graphic design</b> and <b>motion design.</b></p>
